...
Page({
  data: {
    ...
    campusList: ["江安","华西","望江"],
    // showPopup = 2:
    deviceTypeList: ["笔记本 (Laptop)","台式机 (PC/Mac)","移动手机 (Phone)","其他设备 (Others)"],
    // showPopup = 3:
    brandList: ["苹果 Apple","戴尔 DELL","华为 HUAWEI","联想 Lenovo","华硕 ASUS","神舟 HASEE","三星 SamSung","微软 MicroSoft","惠普 HP","宏碁 Acer","雷蛇 Razer","ThinkPad","外星人 Alienware","雷神 Thunder","NEC","机械师 MACHENIKE","微星 Msi","炫龙 Shinelon","清华同方","Intel","技嘉 GIGABYTE","海尔 Haier","LG","东芝 Toshiba","松下 Panasonic","Terrans Force","富士通 FUJITSU","爱尔轩 AIERXUAN","酷比魔方 CUBE","锡恩帝 CENDE","海鲅 HIPAA","索立信","夏普 Sharp","SO-SOON","其他"],
    // showPopup = 4:
  ...
  navigateToNotice() {
    wx.navigateTo({
      url: "/pages/homePage/needToKnow/index",
      success(res) {
  ...
  submitOrder() {
    if (this.data.qq === "" || this.data.phone === "" || this.data.campusValue==="请选择" || this.data.problemDesc === "") {
      Toast("请先填写完整必要的信息。");
      return;
    }
    Dialog.confirm({
